...
const AnecdotePopup = ({anecdoteId, anecdotes, setNewAnecdotes, closePopup, saveAnecdote}:
                           {
                               anecdoteId: string,
                               anecdotes: Anecdote[],
                               setNewAnecdotes: (anecdotes: Anecdote[]) => void,
                               closePopup: () => void,
                               saveAnecdote: () => void}) => {
    ...
    return (
        <section className="flex justify-between gap-24 h-full w-full fixed top-0 left-0 bg-[rgba(30,30,30,0.83)] px-12 z-10">
            ...
            <div className="flex flex-col gap-6 w-full max-w-[600px] py-40 overflow-y-auto scrollbar-hidden">
                <div className="p-6 bg-white relative">
                    <div onClick={closePopup} className="absolute p-2.5 bg-white rounded-[10px] top-[-60px] left-[0px] cursor-pointer">
                        <X />
                    </div>
                    <div className="flex items-center justify-between border-b-[1px] pb-4">
                        <div className="flex items-center gap-2">
                            <img className="w-[30px] h-[30px] rounded-md object-cover" src={anecdote?.user.image} alt="" />
                    ...
                    <div className="pt-4">
                        ...
                        <div className="text-base font-normal mb-14" dangerouslySetInnerHTML={{ __html: anecdote?.content || '' }} />
                ...
                <div className="p-6 bg-white rounded-[20px] rounded-tr-[20px]">
                    ...
                    <div>
                        {anecdote &&
                            anecdote.comments.map((comment: Comment) => (
                                <Comment key={comment.id} user={comment.user} content={comment.content} date={comment.date} />
                            ))}
                    </div>
                </div>
            </div>
